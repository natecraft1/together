...
extension FeedPost {
    static func createModels(n: Int = 12) -> [FeedPost] {
        ...
        return (0..<n).map { num -> FeedPost in
            ...
            let text = !loremIpsum.isEmpty && num % 4 == 0 ? loremIpsum.removeFirst() : nil
            let images: [String] = (1...14).map { $0 }.shuffled().prefix(Int.random(in: 4...7)).map(String.init)
            let names = ["Dwangela", "Kevin and Stacy", "Jim and Pam", "Ryan and Kelly", "The Flendersons"]
...
extension Invite {
    static func create(n: Int = 4) -> [Invite] {
        var names = ["Dwangela", "Kevin and Stacy", "Jim and Pam", "Ryan and Kelly", "The Flendersons"]
        var d = Date()
